...
from ham.solvers import AVBDSolver  # <--- THE UPGRADE


def main():
    print("--- Reproducing: Holonomic Control (AVBD Solver) ---")

    manifold = Sphere(dim=2)
    factory = RandersFactory(manifold, epsilon=0.01)  # 99% Wind

    key = jax.random.PRNGKey(42)
    net = MetricNet(key, input_dim=3, output_dim=3, hidden_dim=64)

    # SETUP AVBD SOLVER
    # beta=2.0 means stiffness doubles every time error persists (aggressive hardening)
    # max_iters=1000 allows time for the relaxation to tighten
    avbd_solver = AVBDSolver(lr=0.01, beta=2.0, max_iters=1000)

    # Define Constraints for AVBD
    # C(x) = ||x|| - 1 = 0
    ...
    # Note: AVBD handles non-projected init better, but let's start on surface
    ...
    def train_step(params, static, opt_state):
        ...
        def loss_fn(p):
            ...
            def metric_fn(x):
                return factory.forward(x, *model(x))

            # CALL AVBD SOLVER
            final_path = avbd_solver.solve(
                lambda p: discrete_randers_energy(p, metric_fn),
                [sphere_constraint],  # List of constraints
                start_p,
                end_p,
                init_inner,
            )
    ...
    for epoch in range(201):
        ...
        if np.isnan(loss_val):
            print(f"WARNING: NaN at epoch {epoch}. Reverting.")
            params = best_params
            break
        ...
        if epoch % 20 == 0:
            print(f"Epoch {epoch:03d} | Miss Dist: {loss:.4f}")
    ...
    ax.plot(path_np[:, 0], path_np[:, 1], path_np[:, 2], "r-", linewidth=4, label="AVBD Geodesic")
    ...
    ax.set_title("Holonomic Control with AVBD Solver")
    ax.legend()
    plt.savefig("avbd_result.png", dpi=150)
    print("Success! Saved avbd_result.png")
